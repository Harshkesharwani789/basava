...
const ShopDetail = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://basavamart.in/api/cart/add",
        payload,
        {
          headers: {
  ...
  const handleDownloadCatalog = async () => {
    try {
      ...
      const response = await axios.get(
        `https://basavamart.in/api/product/download/${product._id}`,
        {
          responseType: "blob",
        }
      );
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(
          `https://basavamart.in/api/product/getproduct/${id}`
        );
    ...
    <Box sx={{ maxWidth: 1200, margin: "0 auto", p: 3 }}>
      ...
      >
        ...
        >
          ...
          >
            <img
              src={`https://basavamart.in${product.images[currentMainImage]}`}
