...
const ProductList = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "https://basavamart.in/api/product/upload-variants",
        formData,
        {
          headers: {
  ...
  const uploadCatalog = async (productId, file) => {
    ...
    try {
      const response = await fetch(
        `https://basavamart.in/api/product/upload/${productId}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const fetchSubCategory = async () => {
    try {
      const response = await axios.get(
        `https://basavamart.in/api/product/getsubcategory`
      );
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(
        "https://basavamart.in/api/product/getproducts"
      );
  ...
  const handleDownload = () => {
    window.open(
      "https://basavamart.in/api/product/download-template",
      "_blank"
    );
  };
  ...
  const handleDelete = async (productId, variantId) => {
    try {
      ...
      const response = await axios.delete(
        `https://basavamart.in/api/product/deleteproduct/${productId}/variant/${variantId}`
      );
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://basavamart.in/api/product/updateproduct/${currentProduct._id}`,
        data
      );
      if (response.status === 201) {
        ...
        >
          <TableHead>
            <TableRow sx={{ backgroundColor: "#f5f5f5" }}>
              <TableCell>Actions</TableCell> <TableCell>Access</TableCell>
              ...
              product?.variants && product?.variants?.length > 0 ? (
                product?.variants?.map((variant, index) => (
                  ...
                  >
                    ...
                    {/* Safeguarded Calculated Columns */}
                    <TableCell>
                      {(variant?.discountAmt ?? 0).toFixed(2)}
                    </TableCell>
                    <TableCell>
                      {(variant?.afterdiscountPrice ?? 0).toFixed(2)}
                    </TableCell>
                    <TableCell>{variant?.profit ?? 0}%</TableCell>
                    <TableCell>
                      {(variant?.profitMargin ?? 0).toFixed(2)}
                    </TableCell>
                    <TableCell>
                      {(variant?.landingPrice ?? 0).toFixed(2)}
                    </TableCell>
                    <TableCell>{variant?.tax ?? 0}%</TableCell>
                    <TableCell>{(variant?.taxAmt ?? 0).toFixed(2)}</TableCell>
                    <TableCell>
                      {(variant?.finalPrice ?? 0).toFixed(2)}
                    </TableCell>
                    <TableCell>{product?._id}</TableCell>
                  </TableRow>
                ))
              ) : (
                // Display placeholder row if no variants are available
