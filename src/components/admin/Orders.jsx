...
const Orders = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      const response = await axios.get(
        `https://basavamart.in/api/order/report`,
        {
  ...
  const handleViewRecord = async (id) => {
    try {
      const response = await axios.get(
        `https://basavamart.in/api/order/getOrderById/${id}`,
        {
          headers: {
  ...
  const handleSave = async () => {
    try {
      await axios.put(
        `https://basavamart.in/api/order/updateOrderStatus/${selectedOrder._id}`,
        {
          status: newStatus,
        }
      );
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        "https://basavamart.in/api/order/getorder"
      );
