  ...
  const fetchRecord = async (role) => {
    try {
      const response = await axios.get(
        `https://basavamart.in/api/auth/getuser?role=${role}`
      );
      ...
      if (isEdit) {
        try {
          const response = await axios.put(
            `https://basavamart.in/api/auth/updateuser/${newRecord._id}`,
            formData
          );
          setRows((prevRows) =>
            prevRows.map((row) =>
              row._id === newRecord._id ? response.data : row
            )
          );
        ...
        try {
          ...
          const response = await axios.post(
            "https://basavamart.in/api/auth/adduser",
            formData,
            {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `https://basavamart.in/api/auth/deleteuser/${id}`
      );
